...
from model.routed_multimodal_vqa_subclamp_routerlog import RoutedMultimodalVQA
...
def compute_router_entropy_loss(output, valid, lambda_router_entropy):
    router_weights = output["router_weights"][valid]
...
def get_avg_router_weights(output, valid):
    router_weights = output["router_weights"][valid]

    if router_weights.numel() == 0:
        return torch.zeros(3, device=output["scores"].device)

    return router_weights.mean(dim=(0, 1)).detach()
...
def evaluate(model, loader, device):
    ...
    router_sum = torch.zeros(3, device=device)
    router_count = 0
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc="Validation"):
            ...
            loss = criterion(scores, labels_valid)
            ...
            avg_router = get_avg_router_weights(output, valid)
            router_sum += avg_router * bs
            router_count += bs
    ...
    avg_router_weights = (
        router_sum / max(router_count, 1)
    ).detach().cpu().tolist()

    return total_loss / total, correct / total, avg_router_weights
...
def train(args):
    ...
    model = RoutedMultimodalVQA(
        ...
        freeze_bert=False,
        use_sub_clamp=not args.no_sub_clamp,
        max_sub_weight=args.max_sub_weight,
        clamp_only_training=args.clamp_only_training,
    ).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in pbar:
            ...
            current_router = router_sum / max(router_count, 1)
            ...
            pbar.set_postfix(
                {
                    ...
                    "rv": current_router[0].item(),
                    "rs": current_router[1].item(),
                    "ro": current_router[2].item(),
                }
            )
        ...
        val_loss, val_acc, val_router = evaluate(
            model=model,
            loader=val_loader,
            device=device,
        )
        ...
        print(
            "Train router avg | "
            f"video={train_router[0]:.4f}, "
            f"subtitle={train_router[1]:.4f}, "
            f"vcpt={train_router[2]:.4f}"
        )

        print(
            "Val router avg   | "
            f"video={val_router[0]:.4f}, "
            f"subtitle={val_router[1]:.4f}, "
            f"vcpt={val_router[2]:.4f}"
        )
        ...
        log_item = {
            ...
            "val_loss": val_loss,
            "val_acc": val_acc,
            "train_router_video": train_router[0],
            "train_router_sub": train_router[1],
            "train_router_vcpt": train_router[2],
            "val_router_video": val_router[0],
            "val_router_sub": val_router[1],
            "val_router_vcpt": val_router[2],
            "max_sub_weight": args.max_sub_weight,
            "sub_clamp_enabled": not args.no_sub_clamp,
            "clamp_only_training": args.clamp_only_training,
        }
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="results/routed_tvqa_star_zeroshot_subclamp_routerlog",
    )
    ...
    parser.add_argument("--p_sub_drop", type=float, default=0.6)
    parser.add_argument("--p_vcpt_drop", type=float, default=0.2)
    parser.add_argument("--lambda_router_entropy", type=float, default=0.05)

    parser.add_argument("--max_sub_weight", type=float, default=0.55)
    parser.add_argument("--no_sub_clamp", action="store_true")
    parser.add_argument("--clamp_only_training", action="store_true")
